function [secantSand] = detSandNeeded(sandValues,finalDistances, horiDistEnd, horiDistStart,dt, startTime, vertDistStart, vertVeloStart, tend, vertDistEnd)
%Determines whether the balloon will reach the final position with the
%current sand dropped.

%Determines whether the balloon will reach the final position whilst dropping the least amount of sand as
%possible. The code then determines the two 0.5kg
%intervals the required sand drop is within and will calculate
%the final positions of this values and the error 
...
%This will keep repeating until a number of sand value within an error
%of 0.5 is found stored in [secantSand]. 
...
%Determines the x and y axis variables
    ...
    while abs(err1) > 0.5
        ...
        [t,z] = ivpSolver(startTime, [vertDistStart; vertVeloStart], dt, tend, vertDistEnd, 1000, x1);
        vertDistColumn = z(1,:);
